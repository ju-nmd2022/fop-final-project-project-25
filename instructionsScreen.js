...
function instructionsScreen() {
  ...
  let instructionsList = ["Use the arrow keys to move the time traveler in all directions", "Collect all the gears to fix the time machine", "Avoid the deadly obstacles along the way", "After collecting all items go through the appearing door to advance in level", "If you die, the character will start over the game from the 1st level.", "Pass all levels to win"];
  ...
  textAlign(LEFT, TOP);
  textLeading(55);
    text(bullet + instructionsList[i], x, y, 600, 600); // Draw the list item with the bullet point
